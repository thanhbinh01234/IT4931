...
var count_margin = { top: 20, right: 100, bottom: 70, left: 60 },
    ...
    count_height = 500 - count_margin.top - count_margin.bottom

// set the ranges
var count_x = d3.scale.ordinal().rangeRoundBands([0, count_width], 0.05)
var count_y = d3.scale.linear().range([count_height, 0])
// define the axis
var count_xAxis = d3.svg
    .axis()
    .scale(count_x)
    .orient('bottom')
...
d3.json('/data/counts', function (error, data) {
    ...
    var line = d3.svg
        .line()
        .x(function (d) {
            return 9 + count_x(d.Time)
        })
        .y(function (d) {
            return count_y(d.Count)
        })
    ...
    count
        ...
        .on('mouseover', function (d) {
            tooltip_count.transition().duration(50).style('opacity', 0)

            tooltip_count.transition().duration(20).style('opacity', 0.9)
