...
def tweet_count(lines):
    ...
    tweet_cnt = lines.flatMap(lambda line: line.split('---')) \
        ...
        .foreachRDD(lambda x: tweet_cnt_li.extend(x.collect()))
    return tweet_cnt_li
...
def user_count(lines):
    ...
    user_cnt = lines.flatMap(lambda line: line.split('---')) \
        ...
        .foreachRDD(lambda x: user_cnt_li.extend(x.distinct().collect()))
    return user_cnt_li
...
def related_keywords(lines):
    ...
    keywords = lines\
        ...
        .foreachRDD(lambda x: x.toDF(['Keyword', 'Count']).sort(desc('Count')).limit(100).registerTempTable("related_keywords_tmp"))
...
def related_hashtags(lines):
    hashtags = lines.flatMap(lambda line: line.split()) \
        ...
        .reduceByKey(lambda x, y: x + y)\
        .foreachRDD(lambda x: x.toDF(['Hashtag', 'Count']).sort(desc('Count')).limit(100).registerTempTable("related_hashtags_tmp"))
...
def sentiment_analysis(lines, hashingTF, iDF):
    ...
    analysis = lines.map(lambda line: line.split()) \
        ...
        .reduce(lambda x, y: x + y)\
        .foreachRDD(lambda x: pos_cnt_li.extend(x.collect()))
...
def main(sc, db, tracking_word):
    ...
    if related_keywords_tb:
        ...
        related_keywords_pd = related_keywords_pd.sort_values("Count", ascending=0).iloc[
                              0:min(9, related_keywords_pd.shape[0])]
    ...
    related_hashtags_pd = related_hashtags_pd.sort_values("Count", ascending=0).iloc[0:min(9, related_hashtags_pd.shape[0])]
    ...
    data_to_db(db, start_time, tweet_cnt_li, user_cnt_len_li, related_keywords_js, related_hashtags_js, pos_cnt_li,
